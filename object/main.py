...
from window import Ui_MainWindow # 导入detect_ui的界面

output = ["Mobility-impaired individuals detected, slow down!", "Please wear your mask!", "overpass",
          "Traffic Congestion"]
            ...
            for i, det in enumerate(pred):
                if det is not None and len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if label.__contains__("truck") or label.__contains__("car"):
                            x1 = xyxy[0].item()
                            y1 = xyxy[1].item()
                            x2 = xyxy[2].item()
                            y2 = xyxy[3].item()

                            if (y1 >= (-0.234 * x1 + 500) or y2 >= -0.234 * x2 + 500) \
                                    and (y1 >= (0.184 * x1 + 200) or y2 >= (0.184 * x2 + 200)):
                                overpass = True

                        if label.__contains__("no_mask"):
                            no_mask = True
                    ...
                    if wheelchair:
                        single_info = output[0]
                        info_show = info_show + single_info + "\n"
                    if overpass:
                        single_info = output[2]
                        info_show = info_show + single_info + "\n"
                    if no_mask:
                        single_info = output[1]
                        info_show = info_show + single_info + "\n"
        return info_show
